...
import sit.project.model.ProductMockup;
import sit.project.model.UrbanCart;
...
public class AddProductToCartServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        UrbanCart cart = (UrbanCart) session.getAttribute("cart");
        if (cart == null) {
            cart = new UrbanCart();
        ...
        sit.project.model.Product p = ProductMockup.createProduct(getServletContext().getAttribute("products"), prodId);
        LineItem li = new LineItem(p, quantity);

        if (quantity == 0) {
            cart.removeProduct(li);
        } else {
            cart.removeProduct(li);
        }
